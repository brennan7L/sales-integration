...
export class MissiveAPI {
  ...
  static async addCommentToConversation(commentBody) {
    if (!this.checkAvailability()) {
      throw new Error('Missive API not available');
    }

    try {
      await window.Missive.comment(commentBody);
      return true;
    } catch (error) {
  ...
  static formatAnalysisForComment(analysis) {
    if (!analysis) return 'No analysis available';
    
    // Clean up the analysis text for Missive comment
    ...
    // Add header
    const header = '⛏️ GOLD PROSPECTOR ANALYSIS\n' + '='.repeat(35) + '\n\n';
    
    // Add footer
    const footer = '\n\n' + '='.repeat(35) + '\n🤖 Generated by AI Gold Prospector';
